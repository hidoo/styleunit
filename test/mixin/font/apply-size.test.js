...
describe('[DEPRECATED] @mixin font-apply-size(...)', () => {
  ...
  it('should out font-size without rem if defaults $font-enable-relative-size is not true.', async () => {
    const cases = [
      ...
      {
        params: [['$value: 14px'], ['$font-enable-relative-size: false']],
        expected: '.selector{font-size:14px}'
      },
      {
        params: [['$value: "medium"'], ['$font-enable-relative-size: false']],
        expected: '.selector{font-size:14px}'
      }
    ];
  ...
  it('should out font-size with !important if argument $important is true.', async () => {
    const cases = [
      ...
      },
      {
        ...
        expected:
          '.selector{font-size:14px !important;font-size:.875rem !important}'
      },
      {
